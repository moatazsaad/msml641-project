...
TerpLoad uses the saved, fine-tuned DistilBERT model for all workload predictions. The required model artifacts are stored in `results/distilbert_model/`, including the Hugging Face model files and `thresholds.joblib` generated by `src/train_distilbert.py`. The app performs inference only and never retrains on a student request.

For courses without an existing profile, TerpLoad fetches available PlanetTerp reviews, runs DistilBERT inference, and caches the resulting course profile in `data/cache/course_profiles_distilbert.json`. Future requests can reuse the cached profile without rerunning inference.

See [production architecture](docs/architecture.md) for system details and the [release/demo runbook](docs/release_runbook.md) for deployment and demo checks.